...
class PatientController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('delete', '<a href="{{route("patient-delete",["id"=>$id])}}" class="btn btn-danger btn-circle"><i class="fas fa-trash"></i></a>')
                ->rawColumns(['checkbox', 'edit', 'delete'])
                ->make(true);
        }
        return view('patient.list', ['roles' => $roles]);
    }
    ...
    public function store(Request $request)
    {
       ...
       $sync = $user->syncRoles($request->roles);
    ...
    public function show($id)
    {
        try {
            ...
            // Handle exceptions, for example, if the user is not found
